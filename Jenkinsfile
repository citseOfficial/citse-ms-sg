node {
   ...
   stage('Preparation') {
       git branch: 'main' , url: 'https://github.com/citseOfficial/citse-ms-sg'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/citse-ms-sg/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/citse-ms-sg/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/citse-ms-sg/runDeployment.sh'
       }catch (e){
